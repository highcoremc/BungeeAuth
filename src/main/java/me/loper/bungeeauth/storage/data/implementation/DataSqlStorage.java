...
public class DataSqlStorage implements DataStorage {
    ...
    private static final String USER_PASSWORD_SELECT_ID = "SELECT id FROM '{prefix}user_password' WHERE unique_id = ? LIMIT 1";
    private static final String USER_PASSWORD_SELECT = "SELECT unique_id,password,hash_method_type,updated_at,created_at FROM '{prefix}user_password' WHERE unique_id = ? LIMIT 1";
    private static final String USER_PASSWORD_INSERT = "INSERT INTO '{prefix}user_password' (password, hash_method_type, unique_id) VALUES (?, ?, ?)";
        ...
        try (Connection c = this.connectionFactory.getConnection()) {
            ...
            try (PreparedStatement s = c.prepareStatement(query)) {
                ...
                try (ResultSet rs = s.executeQuery()) {
                    ...
                    String registeredHost = rs.getString("registered_host");

                    User user = new User(uniqueId, realname, registeredIp, registeredHost, registeredAt);
    ...
    private void saveUserPassword(@NonNull Connection c, @NonNull String query, @NonNull UserPassword password) throws SQLException {
        try (PreparedStatement s = c.prepareStatement(this.statementProcessor.apply(query))) {
            s.setString(1, password.password);
            s.setString(2, password.hashMethodType.getName());
            s.setString(3, password.uniqueId.toString());
            s.executeUpdate();
        }
    }
