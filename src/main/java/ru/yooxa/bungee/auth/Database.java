...
public class Database {
  ...
  private String getString(String name) { return this.manager.getConfig().getString("Table." + name); }
  ...
  public Object[] loadData(ProxiedPlayer player) throws SQLException { return loadData(player.getName()); }
  ...
  public void saveData(final AuthPlayer player, boolean saveSession) {
    final String query = String.format("INSERT INTO `%s` (`%s`, `%s`, `%s`, `%s`, `%s`, `%s`) VALUES ('%s','%s','%s','%s','%s','%s') ON DUPLICATE KEY UPDATE `%2$s`='%8$s',`%3$s`='%9$s',`%4$s`='%10$s',`%5$s`='%11$s',`%6$s`='%12$s',`%7$s`='%13$s';", new Object[] {
          ...
          getString("ip"), 
          getString("server"), 
          getString("email"), player
          .getName().toLowerCase(), player
          .getHash(), 
          String.valueOf((player.getSession() == -1L || !saveSession) ? 0L : (System.currentTimeMillis() / 1000L)), player
          .getPlayer().getAddress().getAddress().getHostAddress(), 
          (player.getPlayer().getServer() == null) ? "" : player.getPlayer().getServer().getInfo().getName(), player.mail
        });
    ...
    Main.getInstance().getProxy().getScheduler().runAsync(Main.getInstance(), new Runnable() {
          public void run() {
            try {
              ...
              e.printStackTrace();
              Main.getInstance().getLogger().info(String.format("Ошибка при сохранении данных игрока '%s': %s", new Object[] { player
